...
class _UsuariosScreenState extends State<UsuariosScreenCONT> {
  ...
  List<Project5> _projects = [];
  Timer? _completionCheckerTimer;
  List<Project5> _completedProjectsToNotify = [];
...
Future<void> _fetchProjectsData() async {
    ...
    try {
      ...
      setState(() {
        _projects = fetchedProcesses.map((process) => Project5(
...
Future<void> _updateProjectsStatus() async {
  ...
  Widget build(BuildContext context) {
        ...
        onItemTap: (index) {
          ...
          if (index == 0) {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => DashboardCont()),
            );
          } else if (index == 1) {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => ProjectsCont( 
    ...
    errorMessage: _projectsErrorMessage, )),
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => SolicitudesAdminScreen( 
                )),
            );
          }
