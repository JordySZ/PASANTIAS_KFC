...
class UsuariosScreenSIR_USER extends StatefulWidget {
  @override
  _UsuariosScreenState createState() => _UsuariosScreenState();
}

class _UsuariosScreenState extends State<UsuariosScreenSIR_USER> {
  ...
  Future<void> fetchUsuarios() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Filtrar solo usuarios con rol SIR_USER
        final usuariosSIR_USER = data.where((usuario) => usuario['rol'] == 'SIR_USER').toList();

        setState(() {
          usuarios = usuariosSIR_USER.reversed.toList();
  ...
  void _mostrarFormularioAgregarUsuario() {
    ...
    // Solo permitir rol SIR_USER
    String rolSeleccionado = 'SIR_USER';
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        // Mostrar rol SIR_USER como texto fijo (no editable)
                        TextFormField(
                          initialValue: 'SIR_USER',
                          readOnly: true,
                          decoration: InputDecoration(
                            labelText: 'Rol',
                            labelStyle: TextStyle(color: darkGrey),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200 || response.statusCode == 201) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario SIR_USER agregado exitosamente', style: TextStyle(color: backgroundColor)),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario SIR_USER actualizado', style: TextStyle(color: backgroundColor)),
                                      backgroundColor: Colors.green[700],
                                    ),
                                  );
                                } else {
  ...
  void _confirmarEliminacion(String id) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: backgroundColor,
        title: Text('Eliminar usuario SIR_USER', style: TextStyle(color: primaryColor)),
        content: Text('¿Estás seguro de eliminar este usuario SIR_USER?', style: TextStyle(color: textColor)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  Future<void> _eliminarUsuario(String id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Usuario SIR_USER eliminado', style: TextStyle(color: backgroundColor)),
            backgroundColor: Colors.green[700],
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al eliminar usuario SIR_USER', style: TextStyle(color: backgroundColor)),
            backgroundColor: primaryColor,
          ),
        );
      }
  ...
  Widget buildUsuarioItem(dynamic usuario) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text('$correo (SIR_USER)', style: TextStyle(color: darkGrey)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            IconButton(
              icon: Icon(Icons.edit, color: primaryColor),
              tooltip: 'Actualizar usuario SIR_USER',
              onPressed: () => _mostrarFormularioActualizarUsuario(usuario),
            ),
            IconButton(
              icon: Icon(Icons.delete, color: primaryColor),
              tooltip: 'Eliminar usuario SIR_USER',
...
Future<void> _updateProjectsStatus() async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: primaryColor,
        title: Text('Usuarios SIR_USER', style: TextStyle(color: backgroundColor)),
        actions: [
          IconButton(
        ...
        onItemTap: (index) {
          ...
          if (index == 0) {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => DashboardSir()),
            );
          } else if (index == 1) {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => ProjectsSIR( 
    ...
    errorMessage: _projectsErrorMessage,
                  ...
                  children: [
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'Buscar usuarios SIR_USER',
                    ...
                    Expanded(
                      child: usuariosFiltrados.isEmpty
                          ? Center(
                            child: Text(
                              'No hay usuarios SIR_USER registrados que coincidan con la búsqueda.',
                              style: TextStyle(color: textColor),
                            ),
                          )
                          : ListView.separated(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: 'Agregar nuevo usuario SIR_USER',
      ),
    );
  }
}