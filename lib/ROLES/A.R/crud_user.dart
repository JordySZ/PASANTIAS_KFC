...
import 'package:login_app/ROLES/A.R/projectArt.dart';
...
class UsuariosScreenAR extends StatefulWidget {
  @override
  _UsuariosScreenState createState() => _UsuariosScreenState();
}

class _UsuariosScreenState extends State<UsuariosScreenAR> {
  ...
  void initState() {
    super.initState();
    fetchUsuarios();
     _fetchProject4sData(); // Añade esto
  ...
  Future<void> fetchUsuarios() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Filtrar solo usuarios con rol A.R_USER
        final usuariosAR = data.where((usuario) => usuario['rol'] == 'A.R_USER').toList();

        setState(() {
          usuarios = usuariosAR.reversed.toList();
  ...
  void _mostrarFormularioAgregarUsuario() {
    ...
    // Solo permitir rol A.R_USER
    String rolSeleccionado = 'A.R_USER';
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        // Mostrar rol A.R_USER como texto fijo (no editable)
                        TextFormField(
                          initialValue: 'A.R_USER',
                          readOnly: true,
                          decoration: InputDecoration(
                            labelText: 'Rol',
                            labelStyle: TextStyle(color: darkGrey),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200 || response.statusCode == 201) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario A.R_USER agregado exitosamente', style: TextStyle(color: backgroundColor)),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario A.R_USER actualizado', style: TextStyle(color: backgroundColor)),
                                      backgroundColor: Colors.green[700],
                                    ),
                                  );
                                } else {
  ...
  void _confirmarEliminacion(String id) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: backgroundColor,
        title: Text('Eliminar usuario A.R_USER', style: TextStyle(color: primaryColor)),
        content: Text('¿Estás seguro de eliminar este usuario A.R_USER?', style: TextStyle(color: textColor)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  Future<void> _eliminarUsuario(String id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Usuario A.R_USER eliminado', style: TextStyle(color: backgroundColor)),
            backgroundColor: Colors.green[700],
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al eliminar usuario A.R_USER', style: TextStyle(color: backgroundColor)),
            backgroundColor: primaryColor,
          ),
        );
      }
  ...
  Widget buildUsuarioItem(dynamic usuario) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text('$correo (A.R_USER)', style: TextStyle(color: darkGrey)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            IconButton(
              icon: Icon(Icons.edit, color: primaryColor),
              tooltip: 'Actualizar usuario A.R_USER',
              onPressed: () => _mostrarFormularioActualizarUsuario(usuario),
            ),
            IconButton(
              icon: Icon(Icons.delete, color: primaryColor),
              tooltip: 'Eliminar usuario A.R_USER',
...
List<Project4> _projectsFiltered = [];
  ...
  bool _isLoadingProject4s = true;
  String? _projectsErrorMessage;
  List<Project4> _projects = [];
  Timer? _completionCheckerTimer;
  List<Project4> _completedProject4sToNotify = [];
...
Future<void> _fetchProject4sData() async {
    setState(() {
      _isLoadingProject4s = true;
      _projectsErrorMessage = null;
    });
    try {
      final fetchedProcesses = await _apiService.getProcesses();
      setState(() {
        _projects = fetchedProcesses.map((process) => Project4(
        ...
        _isLoadingProject4s = false;
        _calculateProject4Percentages();
      });
  ...
  void _calculateProject4Percentages() {
 ...
 void _startCompletionChecker() {
    _completionCheckerTimer = Timer.periodic(Duration(seconds: 10), (timer) {
      ...
      final completedProject4s = _projects.where((project) {
        try {
      ...
      if (completedProject4s.isNotEmpty) {
        setState(() {
          _completedProject4sToNotify = completedProject4s;
        });
 ...
 void _showCompletionAlert() {
    if (_completedProject4sToNotify.isEmpty || !mounted) return;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      showDialog(
        ...
        barrierDismissible: false,
        builder: (context) => AlertDialog(
          title: Text(_completedProject4sToNotify.length == 1 
              ? '¡Proceso completado!'
              : '¡Procesos completados!'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: _completedProject4sToNotify.map((project) {
                return ListTile(
...
Future<void> _updateProject4sStatus() async {
    for (final project in _completedProject4sToNotify) {
      try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: primaryColor,
        title: Text('Usuarios A.R_USER', style: TextStyle(color: backgroundColor)),
        actions: [
          IconButton(
        ...
        onItemTap: (index) {
          ...
          if (index == 2) {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => ProjectsAr(
    projects: _projectsFiltered,
    apiService: _apiService,
    refreshData: _fetchProject4sData,
    processStatusNotifier: processStatusNotifier,
    isLoading: _isLoadingProject4s,
    errorMessage: _projectsErrorMessage,
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => SolicitudAperturaScreen( )),
            );
          }
        },
      ),
                  ...
                  children: [
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'Buscar usuarios A.R_USER',
                    ...
                    Expanded(
                      child: usuariosFiltrados.isEmpty
                          ? Center(
                            child: Text(
                              'No hay usuarios A.R_USER registrados que coincidan con la búsqueda.',
                              style: TextStyle(color: textColor),
                            ),
                          )
                          : ListView.separated(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: 'Agregar nuevo usuario A.R_USER',
      ),
    );
  }
}