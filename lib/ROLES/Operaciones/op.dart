...
class DashboardPage22 extends StatefulWidget {
  const DashboardPage22({super.key});

  @override
  State<DashboardPage22> createState() => _DashboardPageState();
}

class _DashboardPageState extends State<DashboardPage22> {
  final ValueNotifier<String?> processStatusNotifier = ValueNotifier<String?>(null);
  Timer? _completionCheckerTimer;
  List<Project2> _completedProjectsToNotify = [];
  int _selectedIndex = 0;
  final ApiService _apiService = ApiService();
  List<Project2> _projects = [];
  List<Project2> _projectsFiltered = [];
  ...
  Future<void> _fetchProjectsData() async {
    ...
    try {
      ...
      setState(() {
        _projects = fetchedProcesses.map((process) => Project2(
    ...
    switch (index) {
      ...
      case 1:
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => ProjectsTableOpe(
  ...
  Widget _buildPageContent(int index) {
    switch (index) {
      ...
      case 1: return ProjectsTableOpe(
        projects: _projectsFiltered,
        apiService: _apiService,
        refreshData: _fetchProjectsData,
        processStatusNotifier: processStatusNotifier,
