...
class _UsuariosScreenState extends State<UsuariosScreenSD_USER> {
  ...
  Future<void> fetchUsuarios() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Filtrar solo usuarios con rol SD_USER
        final usuariosSD_USER = data.where((usuario) => usuario['rol'] == 'SD_USER').toList();

        setState(() {
          usuarios = usuariosSD_USER.reversed.toList();
  ...
  void _mostrarFormularioAgregarUsuario() {
    ...
    // Solo permitir rol SD_USER
    String rolSeleccionado = 'SD_USER';
                  ...
                  child: ConstrainedBox(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 10),
                        // Mostrar rol SD_USER como texto fijo (no editable)
                        TextFormField(
                          initialValue: 'SD_USER',
                          readOnly: true,
                          decoration: InputDecoration(
                            labelText: 'Rol',
                            labelStyle: TextStyle(color: darkGrey),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200 || response.statusCode == 201) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario SD_USER agregado exitosamente', style: TextStyle(color: backgroundColor)),
                            ...
                            ElevatedButton(
                              ...
                              onPressed: () async {
                                ...
                                if (response.statusCode == 200) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Usuario SD_USER actualizado', style: TextStyle(color: backgroundColor)),
                                      backgroundColor: Colors.green[700],
                                    ),
                                  );
                                } else {
  ...
  void _confirmarEliminacion(String id) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: backgroundColor,
        title: Text('Eliminar usuario SD_USER', style: TextStyle(color: primaryColor)),
        content: Text('¿Estás seguro de eliminar este usuario SD_USER?', style: TextStyle(color: textColor)),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  Future<void> _eliminarUsuario(String id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Usuario SD_USER eliminado', style: TextStyle(color: backgroundColor)),
            backgroundColor: Colors.green[700],
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al eliminar usuario SD_USER', style: TextStyle(color: backgroundColor)),
            backgroundColor: primaryColor,
          ),
        );
      }
  ...
  Widget buildUsuarioItem(dynamic usuario) {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        subtitle: Text('$correo (SD_USER)', style: TextStyle(color: darkGrey)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            IconButton(
              icon: Icon(Icons.edit, color: primaryColor),
              tooltip: 'Actualizar usuario SD_USER',
              onPressed: () => _mostrarFormularioActualizarUsuario(usuario),
            ),
            IconButton(
              icon: Icon(Icons.delete, color: primaryColor),
              tooltip: 'Eliminar usuario SD_USER',
  ...
  List<Project6> _projectsFiltered = [];
  final ApiService _apiService = ApiService();
  bool _isLoadingProjects = true;
  String? _projectsErrorMessage;
  List<Project6> _projects = [];
  Timer? _completionCheckerTimer;
  List<Project6> _completedProjectsToNotify = [];
...
Future<void> _fetchProjectsData() async {
    ...
    try {
      ...
      setState(() {
        _projects = fetchedProcesses.map((process) => Project6(
...
Future<void> _updateProjectsStatus() async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: primaryColor,
        title: Text('Usuarios SD_USER', style: TextStyle(color: backgroundColor)),
        actions: [
          IconButton(
        ...
        onItemTap: (index) {
          ...
          if (index == 0) {
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => DashboardSD()),
            );
          } else if (index == 1) {
            ...
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => ProjectsSd( 
    ...
    errorMessage: _projectsErrorMessage,
                  ...
                  children: [
                    Padding(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          labelText: 'Buscar usuarios SD_USER',
                    ...
                    Expanded(
                      child: usuariosFiltrados.isEmpty
                          ? Center(
                            child: Text(
                              'No hay usuarios SD_USER registrados que coincidan con la búsqueda.',
                              style: TextStyle(color: textColor),
                            ),
                          )
                          : ListView.separated(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: 'Agregar nuevo usuario SD_USER',
      ),
    );
  }
}